...
for filename in /tmp/{{ .Values.groupName }}/{{ .Values.appName }}/{{ .Values.chartVersion }}/locust-reports/*; do
    ...
    /usr/local/bin/mc cp $filename minio/mybucket1/{{ .Values.groupName }}/{{ .Values.appName }}/{{ .Values.chartVersion }}/locust-reports/
done

# check it should not be empty
echo "mc ls minio/mybucket1/{{ .Values.groupName }}/{{ .Values.appName }}/{{ .Values.chartVersion }}/locust-reports/"
/usr/local/bin/mc ls minio/mybucket1/{{ .Values.groupName }}/{{ .Values.appName }}/{{ .Values.chartVersion }}/locust-reports/
