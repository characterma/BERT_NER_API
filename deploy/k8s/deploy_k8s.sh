...
if [ "$1" == "upgrade" ]; then
	...
	echo "helm install --set ciBuildRef=$CI_REF --set appName=$APP_NAME --set imageRepository=$IMAGE_REPOS --set imageTag=$IMAGE_TAG --namespace $K8S_DEPLOY_NAMESPACE -n $RELEASE_NAME ./$CHART_NAME"
